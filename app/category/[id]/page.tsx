...
const categories: Record<string, { name: string; description: string }> = {
  '1': { name: '영양제', description: '일상 건강 관리를 위한 필수 영양제 가이드' },
  '2': { name: '울동/홈트', description: '효과적인 울동과 홈트레이닝을 위한 추천 제품' },
  '3': { name: '다이어트', description: '건강한 체중 관리를 위한 식단과 제품' },
  '4': { name: '뷰티', description: '피부 건강을 위한 스킨케어와 영양제' },
};
...
export default function CategoryPage({ params }: { params: { id: string } }) {
  const category = categories[params.id] || { name: '카테고리', description: '' };
