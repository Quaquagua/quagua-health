    ...
    const fetchPost = async () => {
      ...
      try {
        const res = await fetch(
          `https://mediumturquoise-spider-328427.hostingersite.com/wp-json/wp/v2/posts?slug=${slug}&_embed`
        );
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/" className="text-emerald-600 hover:underline">홈으로 돌아가기</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link to="/" className="text-2xl font-bold text-emerald-600">Quagua Health</Link>
        </div>
      </header>
